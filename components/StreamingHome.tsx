...
function CatalogHome({
  ...
  const beginPremiumCheckout = async () => {
    ...
    try {
      ...
      setPremiumMessage(error instanceof Error ? error.message : "Checkout could not be started.");
  ...
  useEffect(() => {
    ...
    void refreshPremium();
    const timers = [2000, 5000, 9000].map((delay) => window.setTimeout(() => void refreshPremium(), delay));
    ...
    <main className="min-h-screen overflow-hidden bg-krzene-bg">
      <header className="pwa-home-header fixed left-1/2 z-50 flex h-[62px] w-[calc(100%_-_48px)] max-w-[1180px] -translate-x-1/2 items-center justify-between gap-[18px] rounded-[22px] border border-white/9 bg-[rgba(12,12,12,.72)] p-[8px_10px] shadow-[0_16px_50px_rgba(0,0,0,.32)] backdrop-blur-3xl max-[760px]:h-[60px] max-[760px]:w-[calc(100%_-_24px)] max-[760px]:gap-2 max-[760px]:p-[7px_8px]">
        ...
        <div className="flex items-center gap-[7px] border-l border-white/9 pl-3 max-[760px]:border-0 max-[760px]:p-0">
          ...
          <button
            type="button"
            className="h-10 cursor-pointer rounded-xl border border-white/9 bg-white/5 px-3 text-xs font-extrabold text-[#d7d2cb] transition hover:bg-white/12 hover:text-white max-[760px]:hidden"
        ...
        {showMobileMenu && (
          <div className="ui-menu-enter absolute top-[calc(100%_+_8px)] right-0 hidden w-[220px] origin-top-right flex-col gap-1 rounded-2xl border border-white/10 bg-[rgba(14,14,14,.98)] p-2 shadow-[0_24px_70px_rgba(0,0,0,.65)] backdrop-blur-2xl max-[760px]:flex" role="menu">
            ...
            >
              <span className="flex h-5 w-5 items-center justify-center rounded-full border border-current text-[11px]">K</span>
      ...
      {showPremium && (
        <div
          ...
          onMouseDown={(event) => {
            if (event.target === event.currentTarget && !purchasingPremium) setShowPremium(false);
          }}
        >
          <section className="ui-modal-panel-enter relative w-full max-w-[460px] overflow-hidden rounded-[24px] border border-white/10 bg-[linear-gradient(145deg,#1a1917,#0b0b0b)] p-8 shadow-[0_35px_110px_rgba(0,0,0,.8)] max-[560px]:px-5 max-[560px]:py-7" role="dialog" aria-modal="true" aria-labelledby="premium-title">
            <div className="pointer-events-none absolute -top-24 -right-20 h-56 w-56 rounded-full bg-[#e7b64d]/12 blur-3xl" />
            <button type="button" className="absolute top-4 right-4 flex h-9 w-9 cursor-pointer items-center justify-center rounded-full border border-white/9 bg-white/6 text-[#aaa6a0] transition hover:bg-white/13 hover:text-white disabled:opacity-50" onClick={() => setShowPremium(false)} disabled={purchasingPremium} aria-label="Close Premium dialog">
              <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6 6 18" /></svg>
            </button>
      ...
      <div className={`relative z-[4] px-[max(64px,calc((100vw_-_1310px)/2))] pb-[100px] max-[760px]:px-5 max-[760px]:pb-[120px] ${searchMode ? "min-h-screen bg-[#090909] pt-[170px] max-[760px]:pt-[150px]" : "-mt-[150px] bg-[linear-gradient(to_bottom,transparent_0px,rgba(7,7,7,.72)_105px,#090909_220px)] pt-[90px] max-[760px]:-mt-[140px] max-[760px]:pt-[82px]"}`}>
        ...
        {trimmedQuery ? (
          ...
          <>
            ...
            <AdSenseUnit slot={process.env.NEXT_PUBLIC_ADSENSE_SLOT_CATALOG} enabled={adsEnabled} />
      ...
      <div className="mx-auto w-[calc(100%_-_64px)] max-w-[1310px] max-[760px]:w-[calc(100%_-_40px)]">
        <AdSenseUnit slot={process.env.NEXT_PUBLIC_ADSENSE_SLOT_FOOTER} enabled={adsEnabled} />
      </div>
      ...
      <footer className="pwa-footer-safe mx-auto w-[calc(100%_-_64px)] max-w-[1310px] border-t border-white/9 pt-[52px] pb-[30px] text-[#88847e] max-[760px]:mb-6 max-[760px]:w-[calc(100%_-_40px)] max-[760px]:pt-[38px]">
        <div className="grid grid-cols-[minmax(260px,1.7fr)_repeat(3,minmax(120px,.7fr))] gap-12 pb-[45px] max-[760px]:grid-cols-2 max-[760px]:gap-[34px_20px]">
          ...
          <div className="flex flex-col items-start gap-[11px] [&_button]:cursor-pointer [&_button]:border-0 [&_button]:bg-transparent [&_button]:p-0 [&_button]:text-left [&_button]:text-xs [&_button]:text-[#77736e] [&_button]:transition-colors hover:[&_button]:text-white">
            ...
            <button onClick={() => setShowPremium(true)}>{isPremium ? "Premium active" : "Remove ads"}</button>
          </div>
        ...
        <div className="grid grid-cols-[1fr_minmax(280px,1.5fr)_1fr] items-center gap-5 border-t border-white/6 pt-6 text-[10px] max-[760px]:grid-cols-1 max-[760px]:items-start [&_p]:m-0 [&_p]:text-center max-[760px]:[&_p]:text-left">
          ...
          <p className="flex flex-wrap justify-center gap-x-4 gap-y-2 max-[760px]:justify-start"><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link><Link href="/contact">Contact</Link><Link href="/copyright">Copyright</Link></p>
